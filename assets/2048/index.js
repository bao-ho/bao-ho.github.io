...
var swipeSound = new Audio(
  "/assets/2048/Sounds/zapsplat_foley_stick_bendy_whoosh_air_fast_001_12681.mp3"
);
var mergeSound = new Audio(
  "/assets/2048/Sounds/zapsplat_sport_afl_australia_football_catch_hands_001.mp3"
);
var styleSheetNames = [
  "/assets/2048/Styles/classic_style.css",
  "/assets/2048/Styles/vintage_style.css",
];
...
var tileFontSizes = [
  ...
  "2.5rem",
  "2.5rem",
  "2.5rem", //32, 64, 128, 256
  "2.5rem",
  "2rem",
  "2rem",
  "2rem", //512, 1024, 2048, 4096
  "2rem",
]; //8192
...
var mobileTileFontSizes = [
  ...
  "1.25rem",
  "1.25rem",
  "1.25rem", //32, 64, 128, 256
  "1.25rem",
  "1rem",
  "1rem",
  "1rem", //512, 1024, 2048, 4096
  "1rem",
]; //8192
...
var classicBackgroundColors = [
  ...
  "#eb5837",
  "#f4d86b",
  "#efca63", //32, 64, 128, 256
  "#e6c12b",
  "#efc642",
  "#edc501",
  "#5eda92", //512, 1024, 2048, 4096
  "#24bc67",
]; //8192
var classicTextColors = [
  ...
  "#f9f6f2",
  "#f9f6f2",
  "#f9f6f2", //32, 64, 128, 256
  "#f9f6f2",
  "#f9f6f2",
  "#f9f6f2",
  "#f9f6f2", //512, 1024, 2048, 4096
  "#f9f6f2",
]; //8192
...
var vintageBackgroundColors = [
  ...
  "#ae4636",
  "#9fa987",
  "#7a9377", //32, 64, 128, 256
  "#727d6f",
  "#5e7972",
  "#31544f",
  "#563838", //512, 1024, 2048, 4096
  "#3f2a1d",
]; //8192
var vintageTextColors = [
  ...
  "#fff1e1",
  "#3f3232",
  "#3f3232", //32, 64, 128, 256
  "#fff1e1",
  "#fff1e1",
  "#fff1e1",
  "#fff1e1", //512, 1024, 2048, 4096
  "#fff1e1",
]; //8192
...
function init() {
  if (!localStorage.highscore) {
    ...
    localStorage.highscore = Math.max(
      ...
      Number(gameStatus.score)
    );
  }

  document.getElementById("info-best-score").innerHTML =
    "Best<br>" + "<b>" + localStorage.highscore + "</b>";
...
function toggleVolume() {
  if (gameStatus.sound === true) {
    ...
    document.getElementsByClassName("fas fa-volume-up")[0].className =
      "fas fa-volume-mute";
  } else {
    gameStatus.sound = true;
    document.getElementsByClassName("fas fa-volume-mute")[0].className =
      "fas fa-volume-up";
  }
}
...
function toggleStyleSheet() {
  ...
  document
    .getElementById("local-style-sheet")
    .setAttribute("href", styleSheetNames[gameStatus.styleIndex]);

  tileBackgroundColors =
    gameStatus.styleIndex === 0
      ? classicBackgroundColors
      : vintageBackgroundColors;
  tileTextColors =
    gameStatus.styleIndex === 0 ? classicTextColors : vintageTextColors;
  reDrawTiles();
}
...
function toggleDarkMode() {
  ...
  document
    .getElementsByClassName("game-body")[0]
    .classList.toggle("game-dark-mode");
}
...
function reDrawTiles() {
  for (let i = 0; i < gridNodeElements.length; i++) {
    if (nodes[i].tile !== null) {
      //Update positions
      let top = nodes[i].gridNodeElement.offsetTop;
      let left = nodes[i].gridNodeElement.offsetLeft;
      let width = nodes[i].gridNodeElement.offsetWidth;
      let height = nodes[i].gridNodeElement.offsetHeight;
...
function generateRandomTile() {
  ...
  nodes[i].tile.style.fontSize =
    tileFontSizes[Math.log2(nodes[i].tile.innerHTML) - 1];
...
function moveTile(fromNode, toNode) {
  if (toNode.tile === null) {
    ...
    let top = toNode.gridNodeElement.offsetTop;
    let left = toNode.gridNodeElement.offsetLeft;
    ...
    toNode.tile.style.backgroundColor =
      tileBackgroundColors[Math.log2(toNode.tile.innerHTML) - 1];
    toNode.tile.style.color =
      tileTextColors[Math.log2(toNode.tile.innerHTML) - 1];
    toNode.tile.style.fontSize =
      tileFontSizes[Math.log2(toNode.tile.innerHTML) - 1];
    toNode.tile.dataset.alreadyMerge = 1;
    gameStatus.score += parseInt(toNode.tile.innerHTML);
    document.getElementById("info-score").innerHTML =
      "Score<br>" + "<b>" + gameStatus.score + "</b>";
    if (navigator.platform !== "iPhone") {
      toNode.tile.animate(
        [
          // keyframes
    ...
    gameStatus.highestTile = Math.max(
      gameStatus.highestTile,
      toNode.tile.innerHTML
    );
    if (
      gameStatus.highestTile == gameStatus.winningScore &&
      gameStatus.alreadyWon == false
    ) {
